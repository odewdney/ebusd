  ...
  while (!m_shutdown) {
    ...
    if (request.length() > 0) {
      ...
      result_t result = decodeMessage(request, netMessage->isHttp(), &connected, &listening, &user, &reload, &ostream);
		if (!netMessage->isHttp() && (ostream.tellp() == (std::ostringstream::pos_type(0)) || result != RESULT_OK)) {
        ostream.str("");
...
void MainLoop::notifyDeviceData(symbol_t symbol, bool received) {
  ...
  if (symbol != SYN) {
    ...
    if (m_logRawBuffer.tellp() == (std::ostringstream::pos_type) 0 || received != m_logRawLastReceived) {
